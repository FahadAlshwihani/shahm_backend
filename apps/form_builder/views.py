...
from apps.form_builder.actions import FORM_ACTIONS
from apps.services.access.security import (
...
from apps.services.utils.masking import (
    mask_email,
)
    ...
    def get(self, request):
        forms = (
            ...
            .prefetch_related(
                Prefetch(
                    "sections",
                    queryset=FormSection.objects.filter(is_active=True).prefetch_related(
                        Prefetch(
                            "fields",
                            queryset=FormField.objects.filter(is_active=True)
                            .prefetch_related(
                                Prefetch(
                                    "options",
                                    queryset=FormFieldOption.objects.filter(is_active=True).order_by("order", "id"),
                                )
                            )
                            .order_by("order", "id"),
                        )
...
class AdminFormFieldListCreateView(APIView):
    ...
    def post(self, request, form_id):
        ...
        key = incoming.get("key") or incoming.get("system_key")
        if not key:
            key = (
                    incoming.get("label_en")
                    or incoming.get("label_ar")
                    or "field"
            )
        ...
        if deleted_field:
            for attr, value in incoming.items():
                setattr(deleted_field, attr, value)

            deleted_field.section = section
            deleted_field.key = normalized_key
            deleted_field.is_active = True
            deleted_field.save()
