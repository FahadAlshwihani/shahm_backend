...
FILE_SIGNATURES = {
    ...
    ".docx": ((b"PK\x03\x04", b"PK\x05\x06", b"PK\x07\x08"), "DOCX"),
}
...
def validate_uploaded_file(
    ...
    signature_rule = FILE_SIGNATURES.get(extension)

    if signature_rule:
        signatures, label = signature_rule
        if not file_signature.startswith(signatures):
            raise ValidationError(f"Invalid {label} file.")

    if extension == ".wav" and not (
        file_signature.startswith(b"RIFF")
        and file_signature[8:12] == b"WAVE"
    ):
        raise ValidationError("Invalid WAV file.")

    if extension == ".mp3" and not (
        file_signature.startswith(b"ID3")
        or (
            len(file_signature) >= 2
            and file_signature[0] == 0xFF
            and file_signature[1] & 0xE0 == 0xE0
        )
    ):
        raise ValidationError("Invalid MP3 file.")
